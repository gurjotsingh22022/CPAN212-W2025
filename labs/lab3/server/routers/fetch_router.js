...
router.get("/single", (req, res) => {
  ...
  console.log(path.join(upload_directory, filename))
...
router.get("/multiple", (req, res) => {
  ...
  let filename = _.sampleSize(files_array, 3);
  const urls = filename.map(file => path.join(upload_directory, file));
  const images = urls.map((imagePath) => {
    const fileData = fs.readFileSync(imagePath);
    return {
      name: path.basename(imagePath),
      data: fileData.toString('base64'), // Convert to base64
